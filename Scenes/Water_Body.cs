...
public class Water_Body : Node2D
{
    ...
    // #the spring array
    // var springs = []
    List<Node2D> springs = new List<Node2D>();
    // var passes = 12
    int passes = 12;
    ...
    {
        Connect(nameof(ready), this, nameof(r_water_lenght));
        Connect(nameof(ready), this, nameof(r_target_height));
        Connect(nameof(ready), this, nameof(r_bottom));
        Connect(nameof(ready), this, nameof(r_water_polygon));
        Connect(nameof(ready), this, nameof(r_water_border));
        Connect(nameof(ready), this, nameof(r_collisionShape));
        Connect(nameof(ready), this, nameof(r_water_body_area));
        ...
        for (int i = 0; i < spring_number; i++)
        {
            ...
            var w = water_spring.Instance() as Node2D;
            // 		add_child(w)
            AddChild(w);
            // 		springs.append(w)
            springs.Add(w);
            // 		w.initialize(x_position, i)
            // w.Call("initialize", x_position, i);
